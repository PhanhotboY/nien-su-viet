...
import (
	...
	messagingTypes "github.com/phanhotboy/nien-su-viet/libs/pkg/core/messaging/types"
...
func (r *rabbitMQConsumer) handle(
	...
	} else if ack != nil && r.rabbitmqConsumerOptions.AutoAck == false {
		ack()
	}
}
...
func (r *rabbitMQConsumer) deserializeData(
	...
	if contentType == "application/json" {
		...
		r.logger.Errorf(
			fmt.Sprintf(
				"error in deserializing of type '%s' in the consumer: %v",
				r.rabbitmqConsumerOptions.ConsumerMessageType.Name(),
				err,
			),
		)
	}
